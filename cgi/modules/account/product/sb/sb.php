...
class mod_account_product_sb extends mod_account_product
{
	protected $m_name = 'sb';
	protected $order_info;
	
	const WPRO_ACCOUNT_MEDIA_PATH = 'acnt';
	...
	public function pre_output()
	{
		...
		if (!empty($_REQUEST['aid'])) {
			g::$group_id = $_REQUEST['aid'];
		}
		else if (!empty($_REQUEST['oid'])) {
			g::$group_id = $_REQUEST['oid'];
		}
		
		if (!empty($_REQUEST['cid'])) {
			g::$client_id = $_REQUEST['cid'];
		}
		
		if (g::$group_id) {
			$this->order_info = $this->account->to_array();
	...
	protected function index_print_pre_big_buttons_hook()
	{
		echo '<a href="'.cgi::href('account/product/sb/order_details?aid='.$this->account->id).'">Old School</a><br />';
		
		if ($this->order_info['plan']=="Express"){
			echo '<a href="'.cgi::href('account/product/sb/express_details?aid='.$this->account->id).'">Express Details</a><br /><br />';
		}
	}
	...
	protected function get_plan_cost($plan){
        ...
        protected function get_relationships_from_ad($id){
            $relationships = db::select("SELECT * from eppctwo.sb_ad_relationship WHERE ad_id = {$id}");
            $list_array = array();
        ...
        protected function get_location_from_ad(&$list, $type, $id){
            $list_array = array();
            if($type == "city"){
                $cities = db::select("SELECT * from eppctwo.sb_ad_location WHERE ad_id = {$id} AND city <> ''", 'ASSOC');
                //cgi::print_r($cities);
                ...
                $states = db::select("SELECT * from eppctwo.sb_ad_location WHERE ad_id = {$id} AND city = ''", 'ASSOC');
        ...
        public function get_excel_grp_data($grp_id, &$data){
            ...
            $ads = db::select("SELECT * from eppctwo.sb_ads WHERE group_id = ".$grp_id, 'ASSOC');
            ...
            if($group['processed']!="canceled" && $group['processed']!="deleted"){
                db::update(
                	"eppctwo.sb_groups",
                	array('processed' => 'processed', 'edit_status' => 'current'),
                	"id = ".$grp_id
                );
            } else {
                db::update(
        ...
        public function export_excel(){
            ...
            if(isset($_POST['export_type'])){
                switch($_POST['export_type']){
                    case 'all':
                        $orders = db::select("SELECT id from eppctwo.sb_groups WHERE processed <> 'incomplete'");
                        break;
                    case 'new':
                        $orders = db::select("SELECT id from eppctwo.sb_groups WHERE processed = 'new'");
                        break;
                    case 'edited':
                        $orders = db::select("SELECT id from eppctwo.sb_groups WHERE edit_status = 'wpro' && processed <> 'incomplete'");
                        break;
                    case 'current':
                        $orders = db::select("SELECT id from eppctwo.sb_groups WHERE edit_status = 'current' && processed <> 'incomplete'");
                        break;
                    default:
        ...
        public function download_images(){
            $url = 'http://'.\epro\WPRO_DOMAIN.'/'.self::WPRO_ACCOUNT_MEDIA_PATH.'/sb_download_images/';
            if(isset(g::$group_id)){
        ...
        public function upload_image(){
            if(isset($_FILES['new_image']['name'])){
                ...
                $_FILES['new_image']['account_id'] = g::$group_id;
                util::wpro_post('account', 'sb_upload_image', $_FILES['new_image']);
                feedback::add_success_msg("Image uploaded");
            }
        }
        ...
        public function delete_contact(){
            ...
            db::update(
            	"eppctwo.sb_groups",
            	array('processed' => 'deleted', 'edit_status' => 'wpro', 'run_status' => 'paused'),
            	"client_id = ".g::$client_id
            );
        }
         ...
         public function destroy_contact(){
            ...
            $sql = "SELECT * from eppctwo.sb_groups WHERE client_id = ".g::$client_id;
            $groups = db::select($sql,"ASSOC");

            foreach($groups as $group){
                db::delete("eppctwo.sb_ads", "group_id = ".$group['id']);
                db::delete("eppctwo.sb_groups", "id = ".$group['id']);
            }

            db::delete("eppctwo.sbs_payment", "client_id = ".g::$client_id);

            //completly delete all records of this client
            db::delete("eppctwo.clients", "id = ".g::$client_id);
            db::delete("eppctwo.clients_sb", "client = ".g::$client_id);
            db::delete("eppctwo.contacts", "client_id = ".g::$client_id);
        ...
        public function cancel_order(){
            ...
            //$sql = "DELETE from eppctwo.sb_payments WHERE group_id = ".g::$group_id." AND status = 'PENDING'";
            //db::delete($sql);
        }
                    ...
                    if(!empty($keyword_id)){
                       ...
                       //we do an update on that ad
                        ...
                        if(!empty($keyword['text'])){
                            db::update('eppctwo.sb_keywords', $update_keyword_vars, $where);
                        } else {
                            db::exec('DELETE from eppctwo.sb_keywords WHERE '.$where.' LIMIT 1');
                        }
        ...
        public function export(){
            $total_orders = db::select("SELECT * from eppctwo.sb_groups");
            $new_orders = db::select("SELECT * from eppctwo.sb_groups WHERE processed = 'new'");
            $edited_orders = db::select("SELECT * from eppctwo.sb_groups WHERE edit_status = 'wpro'");
            $current_orders = db::select("SELECT * from eppctwo.sb_groups WHERE edit_status = 'current'");
        ...
        public function display_settings(){
            $sql = "SELECT * from eppctwo.sb_settings WHERE name = 'default'";
        ...
        public function save_billing_info(){
            ...
            //db::dbg();
            if(db::update_array("contacts", $contact_data, "client_id=".g::$client_id)){
                feedback::add_success_msg('Client info has been updated!');
            } else {
                feedback::add_error_msg('No chages were made!');
            }
        ...
        public function update_user_files(){
            ...
            $sql = "DELETE from eppctwo.sb_client_files WHERE client_id=".g::$client_id;
            db::exec($sql);
        ...
        public function display_client_info(){
            $groups = db::select("
			SELECT *
			from eppctwo.sb_groups WHERE client_id = '".g::$client_id."'
        ...
        ?>
            ...
            <div id="file_access">
                ...
                foreach($files as $file){
                    $checked = '';
                    $has_file = db::select_one("SELECT file_id from eppctwo.sb_client_files WHERE file_id={$file['id']} and client_id=".g::$client_id);
                    if($has_file){
                        $checked = "checked";
                    }
        ...
        public function create_new_ad(){
            ...
            $settings = db::select_row("SELECT * from eppctwo.sb_settings WHERE name = 'default'", "ASSOC");
        ...
        public function display_order_details(){
            ...
            //$ads = "Select ";
            $sql = "SELECT sum(imps) as imps, sum(cost) as cost from eppctwo.sb_data_ads WHERE campaign_id = '".$this->order_info['id']."'";
                ...
                <div id="group_urls">
                    ...
                    <?php if($this->order_info['is_likepage']){
                        $likepage = db::select_row(
                            "SELECT * from eppctwo.sb_likepages
        ...
        public function display_likepage_details(){
            ...
            $likepage = db::select_row("SELECT * from eppctwo.sb_likepages WHERE id = {$_POST['likepage_id']}", "ASSOC");
            $likepage_links = db::select("SELECT * from eppctwo.sb_likepage_links WHERE likepage_id = {$_POST['likepage_id']}", "ASSOC")
            //print_r($likepage);
        ?>
            <input type='hidden' name='oid' value="<?php echo $this->order_info['id']; ?>">
        ...
        private function image_box(){
            ...
            $result = util::wpro_post('account', 'sb_get_account_images', array(
                   'id' => g::$group_id
            ));
        ...
        private function get_group_select($client_id){
            $sql = "SELECT * from eppctwo.sb_groups WHERE contact_id = {$client_id}";
            $groups = db::select($sql,"ASSOC");
            //cgi::print_r($groups);
        ...
        public function display_edit_ad(){
            ...
            $ad_id = $_REQUEST['selected_ad'];
            if($ad_id){
                $sql = "SELECT * from eppctwo.sb_ads WHERE id = '{$ad_id}' LIMIT 1";
                $ad = db::select_row($sql, "ASSOC");
                if(empty($ad)) {
                    exit("Error: Ad with {$ad_id} could not be found!");
                } else {
                    $sql = "SELECT * from eppctwo.sb_data_ads WHERE ad_id = {$ad_id}";
                    $ad_data = db::select_row($sql, "ASSOC");

                    $client_ads = db::select("SELECT * from sbs_client_update WHERE account_id = '{$this->account->id}' AND type = 'sb-ad' AND processed_dt = '0000-00-00 00:00:00' ORDER BY dt DESC", "ASSOC");
                }
        ...
        ?>
            ...
            <?php if(!empty($client_ads)){ 
		    foreach($client_ads as $client_ad){ 
			    $client_updates = json_decode($client_ad['data']);
			    if($client_updates->ad_id!=$ad['id']) continue;
		?>
			<div id="client_changes">
				...
				foreach($client_updates as $field => $value){
				    if($field=="Image"){
					    $value = '<img src="http://'.\epro\WPRO_DOMAIN.'/uploads/sb/'.g::$group_id.'/'.$value.'" alt="" />';
				    }
				    echo "<li>{$field}: {$value}</li>";
				}
			    ?>
			    </ul>
			    <input type="submit" value="Clear Ticket" ticket_id="<?= $client_ad['id'] ?>" a0='clear_client_changes' />
			</div>
		    <?php 
		} 
	    } ?>
        ...
        private static function edit_ad_keywords($ad){
            $keywords = array();
            if(!empty($ad['id'])){
                $sql = "select * from eppctwo.sb_keywords WHERE ad_id = {$ad['id']}
                    ORDER BY id ASC";
        ...
        private static function edit_target_users($ad){
            ...
            if(!empty($ad['id'])){
                //get state locations
                $sql = "select * from eppctwo.sb_ad_location WHERE ad_id = '{$ad['id']}'
                ...
                $sql = "select * from eppctwo.sb_ad_relationship WHERE ad_id = '{$ad['id']}' LIMIT 1";
                ...
                //get set colleges
                $sql = "select * from eppctwo.sb_ad_college WHERE ad_id = '{$ad['id']}'
                ...
                //get set majors
                $sql = "select * from eppctwo.sb_ad_major WHERE ad_id = '{$ad['id']}'
                    ORDER BY major ASC";
                ...
                //get set companies
                $sql = "select * from eppctwo.sb_ad_company WHERE ad_id = '{$ad['id']}'
        ...
        public function save_ad(){
            ...
            $group_id = $_REQUEST['aid'];
            $ad_id = $_REQUEST['selected_ad'];

            $ad_name = db::select_one("select name from eppctwo.sb_ads where id = :id", array('id' => $ad_id));
            ...
            if(isset($_REQUEST['ad'])){
                ...
                db::update('eppctwo.sb_ads',$_REQUEST['ad'],"id='{$ad_id}'");
                
                if($_REQUEST['ad']['title']!='' && empty($ad_name)){
                    db::update('eppctwo.sb_ads', array('name' => $_REQUEST['ad']['title']), "id='{$ad_id}'");
                }
                ...
                if($_REQUEST['ad']['sex'] || $_REQUEST['ad']['min_age'] || $_REQUEST['ad']['max_age']){
                    $wpro_ad_data = array_merge($wpro_ad_data, array(
            ...
            if(isset($_REQUEST['ad_keywords'])){
                ...
                foreach($_REQUEST['ad_keywords'] as $keyword){
                    ...
                    if($keyword['text']!=''){
                        ...
                        $sql = "DELETE from eppctwo.sb_keywords WHERE id='{$keyword['id']}'";
                        db::exec($sql);
                    }
                }
                ...
                if($location_type == 'state'){
                    ...
                    foreach($_REQUEST['ad_state'] as $ad_state){
                        ...
                        if($ad_state['state']!=''){
                            ...
                            if($ad_state['id']!=''){
                                db::exec("eppctwo.sb_ad_location", "id = :id", array('id' => $ad_state['id']));
                            }
                        }
                    }
                    ...
                    foreach($_REQUEST['ad_city'] as $ad_city){
                        ...
                        if($ad_city['city']!='' && $ad_city['state']!=''){
                            ...
                            if($ad_city['id']!=''){
                                db::delete("eppctwo.sb_ad_location", "id = :id", array('id' => $ad_city['id']));
                            }
                        }
                    }
                    //check radius select
                    if(!isset($_REQUEST['ad']['radius'])){
                        db::update("eppctwo.sb_ads", array('radius' => ''), "id = :id", array('id' => $ad_id));
                    }
            ...
            if(isset($_REQUEST['ad_college'])){
                foreach($_REQUEST['ad_college'] as $ad_college){
                    ...
                    if($ad_college['college']!=''){
                        ...
                        if($ad_college['id']!=''){
                            db::delete("eppctwo.sb_ad_college", "id = :id", array('id' => $ad_college['id']));
                        }
                    }
                }
            }
            ...
            if(isset($_REQUEST['ad_major'])){
                foreach($_REQUEST['ad_major'] as $ad_major){
                    ...
                    if($ad_major['major']!=''){
                        ...
                        if($ad_major['id']!=''){
                            db::delete("eppctwo.sb_ad_major", "id = :id", array('id' => $ad_major['id']));
                        }
                    }
                }
            }
            ...
            if(isset($_REQUEST['ad_company'])){
                foreach($_REQUEST['ad_company'] as $ad_company){
                    ...
                    if($ad_company['company']!=''){
                        ...
                        if($ad_company['id']!=''){
                            db::delete("eppctwo.sb_ad_company", "id = :id", array('id' => $ad_company['id']));
                        }
                    }
                }
            }
            ...
            if(count($wpro_ad_data)>1){
                util::wpro_post('account', 'sb_set_ad', $wpro_ad_data);
            }
        ...
        public function save_ad_name(){
            ...
            if($_POST['ad_name']!=''){
                ...
                db::update('eppctwo.sb_ads', $ad, "id=".$ad_id);
            } else {
        ...
        public function save_group_oid(){
            ...
            db::update('eppctwo.sb_groups',$group,"id=".g::$group_id);
        }
        ...
        public function save_likepage_url(){
            ...
            if(db::update('eppctwo.sb_likepages',$likepage,"group_id=".g::$group_id)){
        ...
        public function save_wpro_comments(){
            ...
            if(db::update('eppctwo.sb_groups', $group, "id=".g::$group_id)){
				$this->order_info['wpro_comments'] = $_POST['wpro_comments'];
			}
        }

        public function clear_client_changes(){
            $r = db::update("eppctwo.sbs_client_update", array('processed_dt' => date(util::DATE_TIME)), "id = :id", array('id' => $_POST['ticket_id']));
            if ($r) {
                feedback::add_success_msg('Ticket Cleared');
            }
            else {
                feedback::add_error_msg('Error: '.db::last_error());
            }
        }
        ...
        public function update_grp_status($group_id){
            ...
            db::update("eppctwo.sb_groups", array('edit_status' => $edit_status), "id = :id", array('id' => $group_id));
        }
        ...
        ?>
            ...
            <div class="creative_sample_container droppable">
                    ...
                    <div class="creative_sample_image">
                        <?php
                            if(!empty($ad['image'])){
                                echo '<img src="http://'.\epro\WPRO_DOMAIN.'/'.self::WPRO_ACCOUNT_MEDIA_PATH.'/sb/'.$ad['group_id'].'/'.$ad['image'].'" />';
                            }
                        ?>
                    </div>
        ...
        {
            $language_options = db::select("select * from eppctwo.sb_languages", "ASSOC");
	...
	public function sts_express_update()
	{
		$data = $_POST;
		
		unset($data['_sts_func_']);
		
		sb_exp_contacts::update_all(array(
        ...
        public function sts_client_update_ad(){
            sbs_lib::client_update('sb', $_POST['account_id'], 'sb-ad', array(
		'ad_id' => $_POST['id'],
               //'Link' => $_POST['link'],
               'title' => $_POST['title'],
            ...
            db::exec("UPDATE sb_groups SET edit_status = 'client', last_client_update = '".date(util::DATE_TIME)."' WHERE id = '".$_POST['account_id']."'");
        }
        ...
        public function sts_get_clients_for_ql()
        {
					$clients = db::select("
						...
						from eppctwo.sb_groups g, contacts c
        ...
        public function sts_recover_order_from_wpro(){
           ...
           db::dbg();
            ...
            if(!db::select_one("select id from eppctwo.sb_groups WHERE client_id = {$_POST['client_id']}")){
        ...
        public function move_payment_data(){
            ...
            $payments = db::select("SELECT * from eppctwo.sb_payments", "ASSOC");
            ...
            foreach($payments as $payment){
                ...
                $group = db::select_row("SELECT * from eppctwo.sb_groups WHERE id = {$payment['group_id']}", "ASSOC");
                ...
                db::update('sb_groups', array('last_bill_date' => $payment['paid_date']), 'id='.$payment['group_id']); 
            }
        }

		public function set_next_bill_dates(){
			$groups = db::select("SELECT * from eppctwo.sb_groups WHERE next_bill_date = '0000-00-00' AND (processed = 'new' OR processed = 'processed')", "ASSOC");
			...
			foreach($groups as $group){
				...
				db::update("sb_groups", array('next_bill_date' => $next_bill_date), "id={$group['id']}");
			}
        ...
        public function sync_ad_groups(){
			if($_GET['group_id']){
				$groups = db::select("SELECT * from eppctwo.sb_groups WHERE id = ".$_GET['group_id'], "ASSOC");
			} else {
				$groups = db::select("SELECT * from eppctwo.sb_groups", "ASSOC");
			}
            foreach($groups as $group){
                ...
                if($r['update']){
                    ...
                    $ads = db::select("SELECT * from eppctwo.sb_ads where group_id=".$group['id'], "ASSOC");
                    foreach($ads as $ad){
                        $kwds = db::select("SELECT text from eppctwo.sb_keywords WHERE ad_id=".$ad['id']." ORDER BY id ASC LIMIT 3", "ASSOC");
                        $keywords = array();
                        foreach($kwds as $kwd){
                            $keywords[] = $kwd['text'];
                        }
        ...
        public function update_bill_day(){
            ...
            $groups = db::select("SELECT * from eppctwo.sb_groups WHERE bill_day = 0", "ASSOC");
        ...
        private static function random_string()
	{
        ...
        public function sendInternalEmail($group_id=0,$payment_id=0){
            $group = db::select_row("SELECT * from eppctwo.sb_groups WHERE id=".$group_id,"ASSOC");
            $contact = db::select_row("SELECT * FROM contacts WHERE id=".$group['contact_id'],"ASSOC");
            $payment = db::select_row("SELECT * from eppctwo.sb_payments WHERE id=".$payment_id,"ASSOC");
     ...
     <?php
        ...
        if(isset($_POST['submit'])){
            ...
            $sql =  "select amount, type from eppctwo.sb_payments";
            if(!empty($conditions)){
            ...
            $sql =  "select SUM(cost) from eppctwo.sb_data_ads";
            if(!empty($conditions)){
                $sql .= " WHERE ".implode(" AND ", $conditions);
            }
            ...
            $sql =  "select * from eppctwo.sb_groups";
            if(!empty($conditions)){
                $sql .= " WHERE ".implode(" AND ", $conditions);
            }
	...
	public function display_express_details()
	{
		$sb_exp_contact = new sb_exp_contacts(array('account_id' => $this->account->id));
		
		$ignore_keys = array('account_id', 'is_db_data_set');
	
		$trs = "";
		foreach($sb_exp_contact as $key => $value){
