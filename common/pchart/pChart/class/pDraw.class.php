   ...
   function drawLineChart($Format=NULL)
    {
     ...
     foreach($Data["Series"] as $SerieName => $Serie)
      {
				$LabelFormatType = $Serie['LabelFormatType'];
             ...
             if ( $DisplayValues && $Serie["Data"][$Key] != VOID ) 
              {
               ...
               $this->drawText($X,$Y-$Offset-$Weight-5,$this->scaleFormat($this->format_label($Serie["Data"][$Key], $LabelFormatType),$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align,"FontSize"=>$DisplaySize));
              }
           ...
           foreach($PosArray as $Key => $X)
            {
             if ( $DisplayValues && $Serie["Data"][$Key] != VOID ) 
              {
              	 $this->drawText($X+$DisplayOffset+$Weight,$Y,$this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit),array("Angle"=>270,"R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE,"FontSize"=>$DisplaySize)); }
               ...
               if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
                {
                 ...
                 $Caption    = $this->scaleFormat($this->format_label($Serie["Data"][$Key], $LabelFormatType),$Mode,$Format,$Unit);
                 ...
                 if ( $DisplayPos == LABEL_POS_INSIDE && abs($TxtHeight) < abs($BarHeight) )
                  ...
                  {
                   if ( $Serie["Data"][$Key] >= 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
                   $this->drawText($X+$XOffset+$XSize/2,$Y2-$Offset,$this->scaleFormat($this->format_label($Serie["Data"][$Key], $LabelFormatType),$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align,"FontSize"=>$DisplaySize));
                  }
   ...
   function drawShape($X,$Y,$Shape,$PlotSize,$PlotBorder,$BorderSize,$R,$G,$B,$Alpha,$BorderR,$BorderG,$BorderB,$BorderAlpha)
    {
     ...
     elseif ( $Shape == SERIE_SHAPE_CIRCLE )
  	...
  	function format_label($label, $LabelFormatType = '')
  	{
			// blar we should not be using util here :(
			if (method_exists('util', $LabelFormatType))
			{
				return util::$LabelFormatType($label);
			}
			else
			{
				return $label;
