...
class Asana {
     ...
     */
     public function addAttachmentToTask($taskId, array $data = array()) {
       $mymeType = array_key_exists('mymeType', $data) ? $data['mymeType'] : null;
     ...
     *
     * @param string $workspaceId
     * @param string $opt $tagName
     *                   (@see http://developer.asana.com/documentation/#tags)
     * @return string JSON or null
     */
    public function createTag($workspaceId, $tagName) {
        //check for duplicate tag name
        $existing_tags = json_decode($this->getTags());
        foreach ($existing_tags->data as $i => $tag) {
            if ($tag->name==$tagName){
                return json_encode(array('data' => array('id' => $tag->id)));
            }
        }
        ...
        return $this->askAsana($this->tagsUrl, $data, ASANA_METHOD_POST);
    }
