...
class lphp
{
	...
	function curl_process($data)
	{
		...
		{
			if ($this->debugging) echo "<br>Binary curl method<br>";    
			if ($data["cbin"] == "true")
			{
				if (isset($data["cpath"]))
					$cpath = $data["cpath"];
		...
		{
			if ($this->debugging) echo "<br>php curl method<br>";    
			$ch = curl_init ();
			...
			$result = curl_exec ($ch);
			if (curl_errno($ch) && $this->debugging)
			{
				echo "<br>CURL ERROR: ".curl_error($ch)."<hr>";
			}
