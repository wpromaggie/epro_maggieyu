...
class mod_eppctwo_users extends rs_object
{
	...
	{
		return (class_exists('user') && !mod_eppctwo_users::is_developer()) ? "{$prefix} !is_test_user" : "";
	}
	...
	public static function get_all_users()
	{
		$users = db::select("
			select *
			from users
			".mod_eppctwo_users::get_test_where_clause('where')."
	...
	public static function exists($username){
		$exists = db::query("SELECT COUNT(*) AS num_users FROM `eppctwo`.`users` WHERE username = '{$username}'");
		return $exists;
	}
}
